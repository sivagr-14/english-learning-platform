...
async function ensureFlashcards(userId: string) {
  const words = await database('vocabulary_words').select(
    'id',
    'category_id'
  );
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const categories = await database('flashcard_queue')
        .join('vocabulary_words', 'flashcard_queue.word_id', 'vocabulary_words.id')
        .join('vocabulary_categories', 'vocabulary_words.category_id', 'vocabulary_categories.id')
        .select(
        ...
        .orderBy([{ column: 'vocabulary_categories.track_number' }, { column: 'vocabulary_categories.category_number' }]);
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const query = database('flashcard_queue')
        .join('vocabulary_words', 'flashcard_queue.word_id', 'vocabulary_words.id')
        .join('vocabulary_categories', 'vocabulary_words.category_id', 'vocabulary_categories.id')
        .join('user_progress', 'flashcard_queue.progress_id', 'user_progress.id')
        .leftJoin('vocabulary_lessons', 'vocabulary_words.id', 'vocabulary_lessons.word_id')
        .select(
