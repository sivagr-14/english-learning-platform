...
class ControlManager {
  ...
  async ensureDocker() {
    ...
    if (opened.status !== 0)
      throw new Error("Docker Desktop could not be opened.");
  ...
  infrastructureDiagnostics(service) {
    const status = this.execute("docker", ["compose", "ps", "--all", service]);
  ...
  async waitForContainer({ containerName, label, probeArgs, service }) {
    let lastOutput = "";
  ...
  spawnServices() {
    ...
    const definitions = [
      ...
      },
      {
        ...
        args: [nextBin, "dev", "-H", frontendHost, "-p", String(frontendPort)],
      },
    ];
    this.servicesStopping = false;
    this.serviceFailure = null;
    ...
    this.services = definitions.map(({ name, args, cwd }) => {
      ...
      const capture = (chunk) => {
        ...
        if (recentOutput.length > 12)
          recentOutput.splice(0, recentOutput.length - 12);
      };
  ...
  async runStart({ preserveLogs = false, preserveStartedAt = false } = {}) {
    ...
    try {
      ...
      if (!Number.isFinite(major) || major < 20) {
        throw new Error(
          `Node.js ${process.versions.node} is unsupported; install 20+.`,
        );
      }
...
if (require.main === module) {
  ...
  server.listen(controlPort, controlHost, () => {
    console.log(
      `English Mastery control page: http://localhost:${controlPort}`,
    );
  });
  const shutdown = () => {
    manager.shutdown();
